...
/// Branded video splash shown over the app while it boots. Plays once,
/// full-bleed, then fades away to reveal the app's first resolved route
/// (login, for a logged-out user). Tap anywhere to skip; respects the
...
class _VideoSplashOverlayState extends State<VideoSplashOverlay>
  ...
  Future<void> _init() async {
    try {
      ...
      c.setLooping(false);
      // Muted: this plays automatically with no user gesture, and a splash
      // screen playing sound unprompted would be jarring/blocked anyway.
      c.setVolume(0);
